...
const App = () => {
	...
	const fetchSettings = async () => {
		try {
			const response = await fetch(
				window.SENTIMENT_ANALYZER?.apiUrl + '/settings',
				{
					headers: {
						'X-WP-Nonce': window.SENTIMENT_ANALYZER?.nonce,
					}
				}
			);
	...
	return (
		<div className="sentiment-analyzer-container max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-md">
			<h1 className="text-2xl font-bold text-gray-800 mb-6">Sentiment Analyzer Settings</h1>
			
			<form onSubmit={handleSave} className="space-y-6">
				<div className="mb-6">
					<h2 className="text-lg font-semibold text-gray-700 mb-3">Keyword Settings</h2>
					<p className="text-gray-600 text-sm mb-4">Enter keywords separated by commas. These will be used to determine sentiment of your content.</p>
					
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
						<div>
							<label className="block text-sm font-medium text-gray-700 mb-1">Positive Keywords</label>
							<textarea
						...
						<div>
							<label className="block text-sm font-medium text-gray-700 mb-1">Neutral Keywords</label>
							<textarea
			...
			<div className="mt-8 pt-6 border-t border-gray-200">
				...
				<p className="text-gray-600">
					This plugin analyzes the sentiment of your WordPress posts based on the keywords you define. 
					Posts are analyzed for positive, negative, or neutral sentiment based on the frequency of matches 
					with your defined keyword lists.
				</p>
			</div>
		</div>
	);
